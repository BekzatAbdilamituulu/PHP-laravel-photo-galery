    ...
    <div class="container mx-auto m-2 p-2 bg-white shadow-md rounded-lg">
        ...
        <form method="POST" action="{{ route('ablums.upload', $album->id) }}" enctype="multipart/form-data">
          ...
          <div class="sm:col-span-6">
            <label for="title" class="block text-sm font-medium text-gray-700"> Загрузить изображения </label>
            <div class="mt-1">
          ...
          <div class="sm:col-span-6 pt-5">
            <x-button class="bg-green-500">Загрузить</x-button>
          </div>
        </form>
       </div>
       <div class="mt-4">
         <div class="grid grid-cols-2 md:grid-cols-3 gap-2 md:gap-4">
           ...
           <div class="bg-gray-300 p-2">
             <a class="block relative h-56 rounded overflow-hidden">
                ...
                src="{{ asset('/storage/'.$photo->id.'/'.$photo->file_name) }}">              </a>
              <div class="flex justify-between mt-4 p-4">
              <a class="m-2 p-2 bg-blue-500 hover:bg-blue-700 rounded-lg" href="{{ route('album.image.show', [$album->id, $photo->id]) }}">Подробнее</a>
              <form method="POST" action="{{ route('album.image.destroy', [$album->id, $photo->id]) }}">
                @csrf
                @method('DELETE')
                <button class="m-2 p-2 rounded-lg bg-red-500 hover:bg-red-700">Удалить</button>  
              </form> 
            </div>
            </div>
           @endforeach
